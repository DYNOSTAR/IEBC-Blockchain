...
const PositionTable = ({ pos, locationLabel, blockchain }) => {
    ...
    return (
        <div className="pos-card" style={{ marginBottom: 20 }}>
            ...
            {sorted.length === 0 && <div className="no-votes">No candidates for this location.</div>}
            {sorted.length > 0 && (
                    ...
                    <table className="results-table">
                        <thead>
                            <tr>
                                ...
                                <th>Candidate</th>
                                <th>Party</th>
                                <th className="t-right">{blockchain?.connected ? 'Chain Votes' : 'Votes'}</th>
                            ...
                            {sorted.map((c, idx) => {
                                ...
                                return (
                                    <tr key={c.id} className={lead ? 'tbl-leader' : ''}>
                                        ...
                                        <td className="t-right" style={{ fontWeight: 700 }}>{v.toLocaleString()}</td>
                                        {blockchain?.connected && <td className="t-right" style={{ color: '#777', fontSize: '.8rem' }}>{c.dbVotes.toLocaleString()}</td>}
                                        <td className="t-right">
                                            <span style={{ fontWeight: 600 }}>{pct}%</span>
                                            <div className="mini-bar-bg"><div className="mini-bar-fill" style={{ width: `${pct}%`, background: partyColor(c.party) }} /></div>
                                        </td>
            ...
            {total === 0 && sorted.length > 0 && <div className="no-votes">No votes recorded yet.</div>}
        </div>
    );
};
...
const SuperAdminResults = () => {
    ...
    const loadResults = useCallback(async (location = {}) => {
        ...
        try {
            ...
            const resultsRes  = await electionAPI.getResults(electionRes.data.election.id, location);
    ...
    useEffect(() => {
        ...
        api.get(`/constituencies/${filterCountyId}`).then(r => { if (r.data?.success) setConstituencies(r.data.constituencies); }).catch(() => {});
    }, [filterCountyId]);

    useEffect(() => {
        setFilterWardId(''); setWards([]);
        if (!filterConstId) return;
        api.get(`/wards/${filterConstId}`).then(r => { if (r.data?.success) setWards(r.data.wards); }).catch(() => {});
    }, [filterConstId]);
    ...
    const integrityStats = useMemo(() => {
        ...
        allPositions.forEach(p => p.candidates.forEach(c => {
            if (c.verified === true) verified++; else if (c.verified === false) mismatched++; else pending++;
        }));
    ...
    if (loading) return (
        <SuperAdminLayout>
            <div className="results-loading" style={{ height: 260 }}>
                <div className="spin-icon">⛓️</div><p>Loading results from blockchain...</p>
            </div>
        </SuperAdminLayout>
    );
    ...
    if (error || !data) return (
        <SuperAdminLayout>
            <div className="results-error" style={{ height: 260 }}>
                <div className="err-icon">⚠️</div><p>{error || 'No results available.'}</p>
        ...
        <SuperAdminLayout>
            <div style={{ padding: '24px' }}>
                ...
                <div className="results-page-head">
                    <div>
                        <div className="results-page-title">Live Results</div>
                        <div className="results-page-subtitle">Kenya General Election · Super Admin View</div>
                    </div>
                ...
                <div className={`bc-banner ${chainActive ? 'ok' : 'warn'}`}>
                    ...
                    <div>
                        <div className="bc-banner-text">
                            {chainActive ? `Blockchain connected — ${blockchain.candidateCount ?? '—'} candidates on chain` : 'Blockchain offline — results unavailable'}
                        </div>
                        <div className="bc-banner-sub">
                            {chainActive ? 'Chain vote counts shown below' : 'Start Ganache to view results.'}
                        </div>
                    </div>
                ...
                {!chainActive && (
                    <div className="location-empty-prompt">
                        ...
                        <div className="loc-prompt-sub">Results are only shown when the Ethereum node is reachable. Start Ganache and ensure the contract is deployed, then refresh.</div>
                        <button className="res-btn green" onClick={() => loadResults(locationRef.current)} style={{ marginTop: 16 }}>↻ Retry</button>
                    </div>
                )}
                ...
                <div className="stats-row">
                    {[
                        ...
                        { icon: '✅', val: integrityStats.verified,               lbl: 'Chain Verified' },
                        { icon: '⚠️', val: integrityStats.mismatched,            lbl: 'Mismatches', highlight: integrityStats.mismatched > 0 },
                    ].map(s => (
                        <div key={s.lbl} className={`stat-card ${s.highlight ? 'red-top' : ''}`}>
                            <div className="stat-icon-r">{s.icon}</div>
                            <div className="stat-num" style={s.highlight ? { color: '#CE1126' } : {}}>{s.val}</div>
                ...
                {presidentialPos && (
                    <>
                        <div className="section-label section-label-national">🇰🇪 Presidential Race — All Kenya</div>
                        <PositionTable pos={presidentialPos} locationLabel="National" blockchain={blockchain} />
                    </>
                )}

                <div className="section-label section-label-local">📍 County, Constituency & Ward Races</div>
                ...
                <div className="location-filter-panel">
                    ...
                    <div className="filter-panel-row">
                        <div className="filter-group">
                            <label>County</label>
                            <select className="filter-select" value={filterCountyId} onChange={e => setFilterCountyId(e.target.value)}>
                                <option value="">— Select County —</option>
                                {counties.map(c => <option key={c.id} value={c.id}>{c.name} County</option>)}
                            </select>
                        </div>
                        <div className="filter-group">
                            <label>Constituency</label>
                            <select className="filter-select" value={filterConstId} disabled={!filterCountyId} onChange={e => setFilterConstId(e.target.value)}>
                        ...
                        <div className="filter-group">
                            <label>Ward</label>
                            <select className="filter-select" value={filterWardId} disabled={!filterConstId} onChange={e => setFilterWardId(e.target.value)}>
                                <option value="">{filterConstId ? '— All —' : 'Select constituency first'}</option>
                                {wards.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                            </select>
                        </div>
                        {filterCountyId && <button className="res-btn outline" onClick={clearFilter} style={{ alignSelf: 'flex-end' }}>✕ Clear</button>}
                    </div>
                    {filterCountyId && (
                        <div className="filter-summary">
                            Showing: {[countyName && `${countyName} County`, constituencyName, wardName && `${wardName} Ward`].filter(Boolean).join(' · ')}
                ...
                {!filterCountyId && (
                    <div className="location-empty-prompt">
                        ...
                        <div className="loc-prompt-sub">Governor, Senator, Women Rep, MP and MCA results are location-specific.</div>
                    </div>
                )}

                {locationRaces.map(pos => (
                    <PositionTable key={pos.positionId} pos={pos} locationLabel={locationLabel(pos.level)} blockchain={blockchain} />
                ))}

                {filterCountyId && !filterConstId && <div className="drill-hint">Select a constituency above to view MP race results.</div>}
                {filterConstId && !filterWardId   && <div className="drill-hint">Select a ward above to view MCA race results.</div>}

                </>}

                <div className="results-foot">
                    Last updated: {lastUpdated} · Auto-refreshes every 30 s · {chainActive ? 'Blockchain counts' : 'Blockchain offline'}
                </div>
            </div>
        </SuperAdminLayout>
    );
